					...
					<a href="<?php echo esc_url($url->getDocumentURLWithUID($content->uid))?>">
								...
								<div class="kboard-light-gallery">
									<!-- <a href="<?php echo esc_url($url->getDocumentURLWithUID($content->uid))?>" class="target-video"> -->
										...
										<?php elseif($content->option->youtube_id):?>
											<div class="revew-latest-slider-container wide" style="background-image:url(<?php echo esc_url($content->option->youtube_thumbnail_url)?>)"></div>
										<?php elseif($content->option->vimeo_id):?>
											<div class="revew-latest-slider-container wide" style="background-image:url(<?php echo esc_url($content->option->vimeo_thumbnail_url)?>)"></div>
										<?php endif?>
									<!-- </a> -->
								</div>
								<!-- <div class="revew-latest-slider-foreground"></div>
								<div class="revew-latest-slider-foreground-search"></div> -->
							<?php elseif(count((array)$content->getAttachmentList()) > 0): $count = 0?>
								<?php foreach($content->getAttachmentList() as $key=>$attach): $extension = strtolower(pathinfo($attach[0], PATHINFO_EXTENSION));?>
									<?php if(in_array($extension, array('mp4')) && $count == 0): $count++?>
										...
										<div class="kboard-light-gallery">
											<div class="revew-latest-slider-container wide target-video" style="background-image:url(<?php echo esc_url($content->getThumbnail(600, 338))?>)" data-poster="<?php echo esc_url($content->getThumbnail(600, 338))?>" data-html="#video-<?php echo $content->uid?>-<?php echo $key?>" ></div>
										</div>
										<!-- <div class="revew-latest-slider-foreground"></div>
										<div class="revew-latest-slider-foreground-search"></div> -->
									<?php endif?>
						...
						<div class="revew-latest-slider-wrap">
							...
							<?php
								...
								?>
								<?php if($info_value):?>
								<div class="revew-latest-slider-info revew-latest-slider-cut-strings">
									<?php echo implode('<span class="kboard-info-separator">ㆍ</span>', $info_value);?>
									<?php if($content->isNew()):?><span class="revew-latest-slider-new-notify new-mark">N</span><?php endif?>
								</div>
							<?php endif?>
