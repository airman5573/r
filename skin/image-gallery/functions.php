...
add_filter("kboard_{$skin_dir_name}_extends_setting", 'kboard_image_gallery_extends_setting', 10, 3);
if(!function_exists('kboard_image_gallery_extends_setting')){
	function kboard_image_gallery_extends_setting($html, $meta, $board_id){
...
add_filter("kboard_{$skin_dir_name}_extends_setting_update", 'kboard_image_gallery_extends_setting_update', 10, 2);
if(!function_exists('kboard_image_gallery_extends_setting_update')){
	function kboard_image_gallery_extends_setting_update($board_meta, $board_id){
...
add_filter('kboard_skin_fields', 'kboard_image_gallery_skin_fields', 10, 2);
if(!function_exists('kboard_image_gallery_skin_fields')){
	function kboard_image_gallery_skin_fields($fields, $board){
		if($board->skin == 'image-gallery'){
			if(isset($fields['media']) && !$fields['media']['description']){
				$fields['media']['description'] = '※ 미디어 추가로 업로드한 이미지는 갤러리에 표시됩니다.';
			}
		}
...
if(!function_exists('kboard_image_gallery_list')){
	function kboard_image_gallery_list($board, $is_latest=false){
		$classes = array();
		
		if($is_latest){
			if(!wp_is_mobile() && $board->meta->latest_row){
				$classes[] = "image-gallery-row-{$board->meta->latest_row}";
			}
		}
		else{
			if(!wp_is_mobile() && $board->meta->pc_row){
				$classes[] = "image-gallery-row-{$board->meta->pc_row}";
			}
			if(wp_is_mobile() && $board->meta->mobile_row){
				$classes[] = "image-gallery-row-{$board->meta->mobile_row}";
			}
		}
