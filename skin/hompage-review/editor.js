...
var kboard_hompage_review_form_submit = false;
...
function kboard_editor_execute(form){
	...
	if(kboard_hompage_review_form_submit){
		return true;
	}
	...
	if(jQuery(form).data('validation') == 'ok'){
		...
		if(jQuery('input[name=kboard_option_vimeo_id]', form).val() && !jQuery('input[name=thumbnail]', form).val() && !jQuery('#kboard-hompage-review-thumbnail').val()){
			jQuery.ajax({
				...
				success: function(data){
					...
					if(!vimeo_thumbnail && confirm(kboard_hompage_review_localize_strings.no_upload)){
						jQuery(form).data('submitted', '');
						jQuery('[type=submit]', form).text(kboard_hompage_review_localize_strings.save);
						jQuery('[type=submit]', form).val('');
					}
					else{
						kboard_hompage_review_form_submit = true;
						jQuery("#kboard-hompage-review-form").submit();
					}
				},
				error: function(){
					if(confirm(kboard_hompage_review_localize_strings.no_upload)){
						jQuery(form).data('submitted', '');
						jQuery('[type=submit]', form).text(kboard_hompage_review_localize_strings.save);
						jQuery('[type=submit]', form).val('');
					}
