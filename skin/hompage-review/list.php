<div id="kboard-hompage-review-list">
	...
	<ul class="kboard-list<?php if(kboard_hompage_review_list($board)):?> <?php echo esc_attr(kboard_hompage_review_list($board))?><?php endif?>">
		...
		<li class="kboard-list-item<?php if($content->uid == kboard_uid()):?> kboard-list-selected<?php endif?>">
				...
				<div class="item-padding">
					<a href="<?php echo esc_url($url->getDocumentURLWithUID($content->uid))?>">
								...
								<div class="kboard-light-gallery">
									<!-- <a href="<?php echo esc_url(kboard_hompage_review_url_with_uid($content->uid))?>" class="target-video"> -->
										<?php if($content->getThumbnail(600, 338)):?>
											<div class="kboard-hompage-review-container wide" style="background-image:url(<?php echo esc_url($content->getThumbnail(600, 338))?>)"></div>
										<?php elseif($content->option->youtube_id):?>
											<div class="kboard-hompage-review-container wide" style="background-image:url(<?php echo esc_url($content->option->youtube_thumbnail_url)?>)"></div>
										<?php elseif($content->option->vimeo_id):?>
											<div class="kboard-hompage-review-container wide" style="background-image:url(<?php echo esc_url($content->option->vimeo_thumbnail_url)?>)"></div>
										<?php endif?>
									<!-- </a> -->
								</div>
							...
							<?php elseif(count((array)$content->getAttachmentList()) > 0): $count = 0?>
								<?php foreach($content->getAttachmentList() as $key=>$attach): $extension = strtolower(pathinfo($attach[0], PATHINFO_EXTENSION));?>
									<?php if(in_array($extension, array('mp4')) && $count == 0): $count++?>
										...
										<div class="kboard-light-gallery">
											<div class="kboard-hompage-review-container wide target-video" style="background-image:url(<?php echo esc_url($content->getThumbnail(600, 338))?>)" data-poster="<?php echo esc_url($content->getThumbnail(600, 338))?>" data-html="#video-<?php echo $content->uid?>-<?php echo $key?>" ></div>
										</div>
										<!-- <div class="kboard-hompage-review-foreground"></div>
										<div class="kboard-hompage-review-foreground-search"></div> -->
										<!-- <div class="enlarge-btn"><i class="xi-search"></i></div> -->
							...
							<?php else:?>
								<!-- <a href="<?php echo esc_url($url->getDocumentURLWithUID($content->uid))?>"> -->
									<?php if($content->getThumbnail(600, 338)):?>
										<div class="kboard-hompage-review-container wide" style="background-image:url(<?php echo esc_url($content->getThumbnail(600, 338))?>)"></div>
									<?php else:?>
										<div class="kboard-hompage-review-container wide no-image"></div>
									<?php endif?>
									<!-- <div class="kboard-hompage-review-foreground"></div>
									<div class="kboard-hompage-review-foreground-search"></div> -->
									<!-- <div class="enlarge-btn"><i class="xi-search"></i></div> -->
						...
						<div  class="kboard-hompage-review-wrap">
						<?php
							$info_value = array();
							...
							<?php if($info_value):?>
							<div class="kboard-hompage-review-info kboard-hompage-review-cut-strings">
								<?php echo implode('<span class="kboard-info-separator">ㆍ</span>', $info_value);?>
								...
								<?php if($content->isNew()):?><span class="kboard-hompage-review-new-notify new-mark">N</span><?php endif?>
							</div>
							<?php endif?>
							<!-- <a class="text-wrap" href="<?php echo esc_url($url->getDocumentURLWithUID($content->uid))?>"> -->
								<div class="kboard-hompage-review-title gallery-title">
	...
	<div class="kboard-control flex-end">
		<a href="<?php echo esc_url($url->getContentEditor())?>" class="kboard-hompage-review-button-small dalia-btn-01"><span class="button-text text-new"><?php echo __('New', 'kboard')?></span></a>
	</div>
	<!-- 버튼 끝 -->
	<?php endif?>
...
wp_enqueue_script('kboard-hompage-review-list', "{$skin_path}/list.js", array('jquery'), KBOARD_VERSION, true);
?>