...
var video_gallery_form_submit = false;
...
function kboard_editor_execute(form){
	...
	if(jQuery(form).data('validation') == 'ok'){
		...
		if(jQuery('input[name=kboard_option_vimeo_id]', form).val() && !jQuery('input[name=thumbnail]', form).val() && !jQuery('#video-gallery-thumbnail').val()){
			jQuery.ajax({
				...
				success: function(data){
					...
					if(!vimeo_thumbnail && confirm(video_gallery_localize_strings.no_upload)){
						jQuery(form).data('submitted', '');
						jQuery('[type=submit]', form).text(video_gallery_localize_strings.save);
						jQuery('[type=submit]', form).val('');
					}
					else{
						video_gallery_form_submit = true;
						jQuery("#video-gallery-form").submit();
					}
				},
				error: function(){
					if(confirm(video_gallery_localize_strings.no_upload)){
						jQuery(form).data('submitted', '');
						jQuery('[type=submit]', form).text(video_gallery_localize_strings.save);
						jQuery('[type=submit]', form).val('');
					}
