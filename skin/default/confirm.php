	...
	<form method="post" action="<?php echo esc_url($url->getConfirmExecute($content->uid))?>">
		...
		<div class="kboard-control">
			<div class="left">
				...
				<a href="<?php echo esc_url($url->getDocumentURLWithUID($content->uid))?>" class="kboard-default-button-small dalia-btn-01"><?php echo __('Document', 'kboard')?></a>
				<?php endif?>
				<a href="<?php echo esc_url($url->getBoardList())?>" class="kboard-default-button-small dalia-btn-01"><?php echo __('List', 'kboard')?></a>
			</div>
			<div class="right">
				<button type="submit" class="kboard-default-button-small dalia-btn-01"><?php echo __('Password confirm', 'kboard')?></button>
			</div>
		</div>
	</form>
</div>