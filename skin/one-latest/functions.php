...
load_plugin_textdomain('one-latest', false, dirname(plugin_basename(__FILE__)) . '/languages');

add_action('wp_enqueue_scripts', 'one_latest_scripts', 999);
add_action('kboard_switch_to_blog', 'one_latest_scripts');
if(!function_exists('one_latest_scripts')){
	function one_latest_scripts(){
		// 번역 등록
		$localize = array(
			'no_upload' => __('The thumbnail could not be loaded automatically. Do you want to upload the thumbnail directly?', 'one-latest'),
			'save' => __('Save', 'kboard')
		);
		wp_localize_script('kboard-script', 'one_latest_localize_strings', apply_filters('one_latest_localize_strings', $localize));
	}
}

add_filter("kboard_{$skin_dir_name}_extends_setting", 'one_latest_extends_setting', 10, 3);
if(!function_exists('one_latest_extends_setting')){
	function one_latest_extends_setting($html, $meta, $board_id){
...
add_filter("kboard_{$skin_dir_name}_extends_setting_update", 'one_latest_extends_setting_update', 10, 2);
if(!function_exists('one_latest_extends_setting_update')){
	function one_latest_extends_setting_update($board_meta, $board_id){
...
if(!function_exists('one_latest_list')){
	function one_latest_list($board, $is_latest=false){
		$classes = array();
...
if(!function_exists('one_latest_url_with_uid')){
	function one_latest_url_with_uid($content_uid){
...
add_filter('kboard_skin_fields', 'one_latest_skin_field', 10, 2);
if(!function_exists('one_latest_skin_field')){
	function one_latest_skin_field($fields, $board){
		if($board->skin == 'hwaikeul-video'){
			if(!isset($fields['youtube_id'])){
				$fields['youtube_id'] = array(
					'field_type' => 'text',
					'field_label' => __('YouTube ID', 'one-latest'),
					'field_name' => __('YouTube ID', 'one-latest'),
					...
					'roles' => array(),
					'default_value' => '',
					'placeholder' => __('YouTube Video ID', 'one-latest'),
					'required' => '',
					'show_document' => '',
					'description' => '',
					'close_button' => 'yes'
			...
			if(!isset($fields['vimeo_id'])){
				$fields['vimeo_id'] = array(
					'field_type' => 'text',
					'field_label' => __('Vimeo ID', 'one-latest'),
					'field_name' => __('Vimeo ID', 'one-latest'),
					...
					'roles' => array(),
					'default_value' => '',
					'placeholder' => __('Vimeo Video ID', 'one-latest'),
					'required' => '',
					'show_document' => '',
					'description' => '',
					'close_button' => 'yes'
			...
			if(!isset($fields['video_url'])){
				$fields['video_url'] = array(
					'field_type' => 'text',
					'field_label' => __('Video URL', 'one-latest'),
					'field_name' => __('Video URL', 'one-latest'),
					...
					'roles' => array(),
					'default_value' => '',
					'placeholder' => __('Video URL', 'one-latest'),
					'required' => '',
					'show_document' => '',
					'description' => '',
					'close_button' => 'yes'
			...
			if(!isset($fields['video_view'])){
				$fields['video_view'] = array(
					'field_type' => 'select',
					'field_label' => __('Screen Ratio', 'one-latest'),
					'field_name' => __('Screen Ratio', 'one-latest'),
					'class' => 'kboard-attr-row kboard-attr-video_view',
					'meta_key' => 'video_view',
					'row' => array(
						''=>array('label'=>__('Wide Screen (16:9)', 'one-latest')),
						'normal'=>array('label'=>__('Standard (4:3)', 'one-latest'))
					),
					'default_value' => '',
					'permission' => '',
					'roles' => array(),
			...
			if(!isset($fields['autoplay'])){
				$fields['autoplay'] = array(
					'field_type' => 'select',
					'field_label' => __('Auto Play', 'one-latest'),
					'field_name' => __('Auto Play', 'one-latest'),
...
add_action('kboard_skin_field_after_youtube_id', 'one_latest_skin_field_after_youtube_id', 10, 3);
if(!function_exists('one_latest_skin_field_after_youtube_id')){
	function one_latest_skin_field_after_youtube_id($field, $content, $board){
		if($board->skin == 'hwaikeul-video'){
			?>
...
add_action('kboard_skin_field_after_vimeo_id', 'one_latest_skin_field_after_vimeo_id', 10, 3);
if(!function_exists('one_latest_skin_field_after_vimeo_id')){
	function one_latest_skin_field_after_vimeo_id($field, $content, $board){
		if($board->skin == 'hwaikeul-video'){
			?>
			<!-- <div class="kboard-attr-row">
				<div class="attr-value">
					<input type="hidden" name="kboard_option_vimeo_thumbnail_url" value="<?php echo esc_url($content->option->vimeo_thumbnai_url)?>">
					<div class="description"><?php echo __('※ Please enter only the ID value at the end of the url.', 'one-latest')?> (<?php echo __('ex', 'one-latest')?>:https://vimeo.com/<span class="text-bold">237551523</span>)</div>
				</div>
			</div> -->
			<?php
		}
...
add_filter('kboard_get_template_field_html', 'one_latest_get_template_field_html', 10, 4);
if(!function_exists('one_latest_get_template_field_html')){
	function one_latest_get_template_field_html($field_html, $field, $content, $board){
		if($board->skin == 'hwaikeul-video'){
				...
				<div class="kboard-attr-row <?php echo esc_attr($field['class'])?> meta-key-<?php echo esc_attr($meta_key)?> <?php echo esc_attr($required)?>">
					...
					<div class="attr-value">
						<select id="<?php echo esc_attr($meta_key)?>" name="<?php echo esc_attr($fields->getOptionFieldName($meta_key))?>"class="<?php echo esc_attr($required)?>">
							<option value=""><?php echo __('Wide Screen (16:9)', 'one-latest')?></option>
							<option value="normal"<?php if($content->option->video_view):?> selected<?php endif?>><?php echo __('Standard (4:3)', 'one-latest')?></option>
						</select>
...
add_action('kboard_skin_field_after_autoplay', 'one_latest_skin_field_after_autoplay', 10, 3);
if(!function_exists('one_latest_skin_field_after_autoplay')){
	function one_latest_skin_field_after_autoplay($field, $content, $board){
		if($board->skin == 'hwaikeul-video'){
			?>
			<!-- <div class="description"><?php echo __('※ Not applicable in all environments, such as mobile.', 'one-latest')?></div> -->
			<?php
		}
	}
}