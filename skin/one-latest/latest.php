<div id="one-latest" class="one-latest<?php if(one_latest_list($board, true)):?> <?php echo esc_attr(one_latest_list($board, true))?><?php endif?>">
	<?php while($content = $list->hasNext()):?>
	<div class="one-latest-item<?php if($content->uid == kboard_uid()):?> kboard-list-selected<?php endif?>">
		<div class="one-latest-thumbnail">
			<div class="item-padding">
				...
				<a href="<?php echo esc_url($url->getDocumentURLWithUID($content->uid))?>" class="target-video">
					<?php if($content->getThumbnail(700, 700)):?>
						<div class="one-latest-container latest target-video" style="background-image:url(<?php echo esc_url($content->getThumbnail(700, 700))?>)"></div>
					<?php elseif($content->option->youtube_id):?>
						<div class="one-latest-container latest target-video" style="background-image:url(<?php echo esc_url($content->option->youtube_thumbnail_url)?>)"></div>
					<?php elseif($content->option->vimeo_id):?>
						<div class="one-latest-container latest target-video" style="background-image:url(<?php echo esc_url($content->option->vimeo_thumbnail_url)?>)"></div>
					<?php endif?>
				</a>
					...
					<?php foreach($content->getAttachmentList() as $key=>$attach): $extension = strtolower(pathinfo($attach[0], PATHINFO_EXTENSION));?>
						<?php if(in_array($extension, array('mp4')) && $count == 0): $count++?>
							...
							<a href="<?php echo esc_url($url->getDocumentURLWithUID($content->uid))?>" class="target-video">
								...
								<div class="one-latest-container latest target-video" style="background-image:url(<?php echo esc_url($content->getThumbnail(700, 700))?>)" data-poster="<?php echo esc_url($content->getThumbnail(700, 700))?>" data-html="#video-<?php echo $content->uid?>-<?php echo $key?>" ></div>
				...
				<?php else:?>
						...
						<a href="<?php echo esc_url($url->getDocumentURLWithUID($content->uid))?>">
							<?php if($content->getThumbnail(700, 700)):?>
								<div class="one-latest-container latest latest" style="background-image:url(<?php echo esc_url($content->getThumbnail(700, 700))?>)"></div>
							<?php else:?>
								<div class="one-latest-container latest no-image"></div>
							<?php endif?>
						</a>
		...
		<div class="thumbnail-text-wrapper">
			<a href="<?php echo esc_url($url->getDocumentURLWithUID($content->uid))?>" title="<?php echo esc_attr($content->title)?>">
				<div class="one-latest-title one-latest-cut-strings">
					<?php if($content->isNew()):?><span class="one-latest-new-notify">N</span><?php endif?>
				...
				<div class="one-latest-date revew-cut-strings">
...
wp_enqueue_script('one-latest', "{$skin_path}/latest.js", array('jquery'), KBOARD_VERSION, true);
?>