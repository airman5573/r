...
if($board->initCategory2()){
	...
	$status_list = kboard_franchise_qna_status();
}
?>
<div id="kboard-franchise-qna-latest">
	<table>
		<thead>
			<tr>
			...
			<tr>
				<td class="kboard-latest-status">
					<?php if($content->category2):?>
						<span class="kboard-franchise-qna-status status-<?php echo array_search($content->category2, $status_list)?>"><?php echo $content->category2?></span>
					<?php endif?>
				</td>
				<td class="kboard-latest-title">
					<a href="<?php echo $url->set('uid', $content->uid)->set('mod', 'document')->toStringWithPath($board_url)?>">
						<div class="kboard-franchise-qna-cut-strings">
							<?php if($content->isNew()):?><span class="kboard-franchise-qna-new-notify">N</span><?php endif?>
